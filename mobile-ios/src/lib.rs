//! iOS-клиент babangida: Dioxus + нативный шелл (ADR-0007 — самая рискованная
...
//! — `dx build`/Xcode под фичей `shell` (нужен device-тулчейн). См. `AGENTS.md`.
...
/// Нативный шелл: запуск под iOS (webview-рендер через wry/tao). Под фичей `shell`
/// — требует device-тулчейн (Xcode/`dx`), не `cargo check`. План Б — ADR-0007.
#[cfg(feature = "shell")]
pub fn launch() {
    dioxus::launch(App);
}
